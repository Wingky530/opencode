       ...
       for (const part of parts) {
         ...
         const input = part.state?.input as Record<string, unknown> | undefined
           if (part.tool === ShellID.ToolID) {
             ...
             result.push({
               id: part.id,
               label: String((input?.command as string) ?? (input?.description as string) ?? "bash"),
               status: "running" as const,
               output: String(part.state?.output ?? "").split("\n").filter(Boolean),
               startedAt: part.time?.created ?? Date.now(),
             })
           } else if (part.tool === "task") {
             const desc = String(input?.description ?? "Task")
             const agent = String(input?.subagent_type ?? "General")
             result.push({
               id: part.id,
               label: `[${agent}] ${desc}`,
